                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.blog.posts.update', $post->id) }}"
                            ...
                            <select
                                ...
                                @foreach ($blogCategories as $blogCategory)
                                    <option value="{{ $blogCategory->id }}"
                                        {{ $post->blogCategory->id === $blogCategory->id ? 'selected' : '' }}>
                                        {{ $blogCategory->name ?? '' }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="lang-group">
                                @foreach (config('lang.locales') as $key => $locale)
                                    <input type="hidden" id="name" name="name:{{ $key }}"
                                        ...
                                        input-lang="{{ $key }}"
                                        value="{{ old('name:' . $key) ?? $post->translate($key)->name }}" required>
                                ...
                                @foreach (config('lang.locales') as $key => $locale)
                                    <div style="display: none;" input-lang="{{ $key }}">
                                        <textarea id="description" name="description:{{ $key }}"
                                            ...
                                            required>
                                        {{ old('description:' . $key) ?? $post->translate($key)->description }}
                                    </textarea>
                                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="tags[]" multiple data-role="tagsinput" required>
                                @foreach ($post->blogTags as $blogTag)
                                    <option value="{{ $blogTag->name }}">{{ $blogTag->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @if ($post->image)
                            <div class="w-25">
                                ...
                                @include('admin2.components.image', ['image' => $post->image])
                            </div>
                        @endif
