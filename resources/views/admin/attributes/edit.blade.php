                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.attributes.update', $attribute->id) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select @error('attribute_group_id') is-invalid @enderror"
                                name="attribute_group_id" id="attribute_group" required>
                            ...
                            <div class="lang-group">
                                @foreach (config('lang.locales') as $key => $locale)
                                    <input type="hidden" id="name" name="name:{{ $key }}"
                                        ...
                                        input-lang="{{ $key }}"
                                        value="{{ old('name:' . $key) ?? $attribute->translate($key)->name }}"
                                        required>
                                ...
                                @foreach (config('lang.locales') as $key => $locale)
                                    <div style="display: none;" input-lang="{{ $key }}">
                                        <textarea id="description" name="description:{{ $key }}"
                                            ...
                                            required>
                                                                {{ old('description:' . $key) ?? $attribute->translate($key)->description }}
                                                            </textarea>
                                    </div>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox"
                                    ...
                                    name="status" value="1" {{ $attribute->status ? 'checked' : '' }}>
