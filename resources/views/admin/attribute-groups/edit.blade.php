        ...
        <div class="col-lg-10 mx-auto">
            <div class="card shadow">
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('admin.attribute-groups.update', $attributeGroup->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="lang-group">
                                @foreach (config('lang.locales') as $key => $locale)
                                    <input type="hidden" id="name" name="name:{{ $key }}"
                                        ...
                                        input-lang="{{ $key }}"
                                        value="{{ old('name:' . $key) ?? $attributeGroup->translate($key)->name }}"
                                        required>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox"
                                    ...
                                    name="status" value="1" {{ $attributeGroup->status ? 'checked' : '' }}>
