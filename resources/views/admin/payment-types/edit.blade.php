        ...
        <div class="col-lg-10 mx-auto">
            <div class="card shadow">
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('admin.payment-types.update', $paymentType->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="lang-group">
                                @foreach (config('lang.locales') as $key => $locale)
                                    <input type="hidden" id="name" name="name:{{ $key }}"
                                        ...
                                        input-lang="{{ $key }}"
                                        value="{{ old('name:' . $key) ?? $paymentType->translate($key)->name }}"
                                        required>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox"
                                    ...
                                    name="status" value="1"
                                    {{ old('status') == '1' || $paymentType->status === 1 ? 'checked' : '' }}>
