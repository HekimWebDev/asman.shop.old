        ...
        <div class="col-lg-10 mx-auto">
            <div class="card shadow">
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST"
                        action="{{ route('admin.attributes.values.update', [$attribute->id, $attributeValue->id]) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="lang-group">
                                @foreach (config('lang.locales') as $key => $locale)
                                    <input type="hidden" id="name" name="name:{{ $key }}"
                                        ...
                                        input-lang="{{ $key }}"
                                        value="{{ old('name:' . $key) ?? $attributeValue->translate($key)->name }}"
                                        required>
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox"
                                    ...
                                    name="status" value="1" {{ $attributeValue->status ? 'checked' : '' }}>
